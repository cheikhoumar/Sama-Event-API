...
<div class="p-4">
	...
	<Heading tag="h2" class="mb-3">Demande de retrait</Heading>
	<div class="mb-3 flex items-center gap-2 text-orange-400 dark:text-orange-300">
		<ExclamationCircleOutline class="inline" />
		<div>
			<span> Le montant a été déduit des frais. </span>
			<br />
			<span>Frais: <strong>{defaultCurrencyFormatter.format(data.withdrawalFee)}</strong></span>
		</div>
	</div>
...
<Table striped>
	<TableHead>
		<TableHeadCell>Date de soumission</TableHeadCell>
		<TableHeadCell>Demandeur</TableHeadCell>
		<TableHeadCell>Statut</TableHeadCell>
		<TableHeadCell>Mode de paiement</TableHeadCell>
		<TableHeadCell>Montant</TableHeadCell>
		<TableHeadCell>Frais</TableHeadCell>
							...
							<div class="flex items-center gap-2">
								<Button
									on:click={() => {
										...
										actionMsg = 'Confirmation du règlement du retrait ?'
									}}
									pill
									size="sm"
									color="green">Confirmer</Button
								>
								<Button
									on:click={() => {
										actionModal = true
										...
										actionWithdrawId = withdraw.id
										actionMsg =
											'Le retrait sera refusé et le solde sera restitué à l\'utilisateur.'
									}}
									pill
									size="sm"
									color="red"
									outline>Rejeter</Button
								>
							</div>
						{:else}
							<Button
								on:click={() => {
									...
									actionCtx = 'cancel'
									actionWithdrawId = withdraw.id
									actionMsg = 'Annuler la confirmation/le rejet du retrait.'
								}}
								pill
								size="sm"
								color="yellow"
