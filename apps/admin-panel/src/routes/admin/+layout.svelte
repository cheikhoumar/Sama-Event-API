			...
			>
				...
				<div class="flex w-full gap-2">
					...
					<Button href="/admin/logout" size="xs" pill color="red">
						<span class="mx-2 font-semibold">Déconnexion</span>
					</Button>
				</div>
			</div>
		</div>
		...
		<Sidebar {activeUrl}>
			<SidebarWrapper divClass="py-4 px-3 rounded dark:bg-gray-800">
				<SidebarGroup>
					...
					<SidebarDropdownWrapper isOpen={activeUrl === '/admin/event'} label="Gestion des événements">
						<svelte:fragment slot="icon">
							<CalendarMonthSolid
								class="h-5 w-5 text-gray-500 transition duration-75 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white"
							/>
						</svelte:fragment>
						<SidebarItem label="En attente" href={'/admin/event?status=DRAFT'} on:click={toggleSide}>
							<svelte:fragment slot="icon">
								<FileSolid
						...
						<SidebarItem label="Supprimé" href={'/admin/event?deleted=true'} on:click={toggleSide}>
							<svelte:fragment slot="icon">
								<TrashBinSolid
									class="ml-3 h-5 w-5 text-gray-500 transition duration-75 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white"
								/>
					...
					<SidebarDropdownWrapper isOpen={activeUrl === '/admin/user'} label="Gestion des utilisateurs">
						<svelte:fragment slot="icon">
							<UserSolid
								class="h-5 w-5 text-gray-500 transition duration-75 group-hover:text-gray-900 dark:text-gray-400 dark:group-hover:text-white"
							/>
